...
export class DatasetGenerator {
  generate(config: GeneratorConfig): EvaluationSample[] {
    ...
    for (let i = 0; i < config.numSamples; i++) {
      ...
      const query = template.queries[queryIdx];
      const contextTemplate = template.contexts[contextIdx];
      const answer = template.answers[answerIdx];
      if (!query || !contextTemplate || !answer) continue;

      const sample: EvaluationSample = {
        query,
        context: this.generateContext(contextTemplate, config.difficulty),
        ground_truth: answer,
  ...
  private shuffle<T>(array: T[]): T[] {
    ...
    for (let i = shuffled.length - 1; i > 0; i--) {
      ...
      [shuffled[i], shuffled[j]] = [shuffled[j] as T, temp];
    }
    return shuffled;
  }
