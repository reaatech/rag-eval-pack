...
export class MetricsEngine {
  ...
  aggregateResults(results: SampleEvalResult[]): AggregatedMetrics {
    ...
    const overallScores = results
      .filter((r): r is typeof r & { overall_score: number } => r.overall_score !== undefined)
   ...
   */
  private mean(values: (number | undefined)[]): number {
   ...
   */
  private stdDev(values: (number | undefined)[]): number {
    const filtered = values.filter((v): v is number => v !== undefined);
    if (filtered.length === 0) return 0;
    const mean = this.mean(filtered);
    const squareDiffs = filtered.map((value) => (value - mean) ** 2);
    const variance = squareDiffs.reduce((a, b) => a + b, 0) / filtered.length;
