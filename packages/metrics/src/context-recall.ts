import type { ContextRecallResult, EvaluationSample, FactCoverage } from '@reaatech/rag-eval-core';
...
export class ContextRecallScorer {
  ...
  private extractKeyPhrases(sentence: string): string[] {
    ...
    if (phrases.length === 0) {
      const words = getSignificantWords(processed.toLowerCase());
      if (words.length > 0) {
  ...
  private async checkFactCoverage(fact: string, context: string): Promise<FactCoverage> {
    ...
    const factWords = getSignificantWords(factLower);
