...
function parseGateConfig(content: string): import('../../types/domain.js').GateConfig[] {
  try {
    const parsed = JSON.parse(content);
    return parsed.gates ?? parsed;
  } catch {
    const parsed = parseYaml(content) as { gates?: import('../../types/domain.js').GateConfig[] };
    return parsed.gates ?? (parsed as import('../../types/domain.js').GateConfig[]);
  }
}

function generateBasicMarkdownReport(results: import('../../types/domain.js').EvalResults): string {
